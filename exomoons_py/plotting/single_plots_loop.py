...
for i, n in enumerate(i_deg):
    for k, m in enumerate(phi_deg):
        # Template: bring_disk_data(impact, i_in, phi_in, output, targetdir, modelnumber, paramfile=False)

        # create data
        print("\nMAKING MODEL 0 NOW:")
        disksim.bring_disk_data(impact, n, m, "model_i" + str(n) + "_phi" + str(m), targetdir, str(0), paramfile=False)
        ...
        time, flux, flux_errxx = bring.lightcurve_import(data + "/model_i" + str(n) + "_phi" + str(m) + '.dat', 'time',
                                                         'flux', 'flux_rms')
        ...
        draw_rings.plotting_rings(plot, in_fits, days_side, -y_range, y_range,
                                  data)  # this fills in the ring system grids
