...
def plotting_rings(rplot,in_fits,days_side,y_min_ring,y_max_ring,targetdir):
    ...
    # read in photometry
    (time, flux, flux_err) = j1407.j1407_photom_binned(targetdir+'/lightcurve_bring.dat', rings_tmin, rings_tmax)   # the last two values are the limits in time units as used in the data file between which I want the photometry to be displayed. They are the begining and end time of the eclipse plus a bit of contingency, which here is the part of the data for the rest of the Hill sphere
    ...
    star_line = mpl.patches.Rectangle((hjd_minr-(trange/2.), res[0]-dstar/2.), trange, dstar, color='g', zorder=-15)   # width of the path should match size of the plot (found further below)
