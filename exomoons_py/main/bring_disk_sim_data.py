...
def bring_disk_data(impact,i_in,phi_in,output,targetdir,modelnumber,paramfile=False):
    ...
    masses_array = np.array([m_io,m_europe,m_ganymede])   # if special case, change here (but not with scaling factors, that comes later)
    ...
    axes_array = np.array([a_io,a_europe,a_ganymede])                   # if special case, change here (but not with scaling factors, that comes later)
    ...
    super_table = Table([out0,out1,out2,out2c,out2a,out2b,out3,out3a,out3b,out4,out4a],names=['name','ID','mass[kg]','mass[mjup]','mass[m_Picb]','mass[mearth]','axis[m]','axis[R_H planet]','axis[r_Picb]','R_Hill[m]','R_Hill[r_Picb]'], meta={'name': 'supermoons'})
    ...
    days_array_start = np.linspace(0.,day_no,day_no*288.)   # [days], in 5min intervals (there are 288 5min intervals in a day)
    ...
    radii.append(transit_days/4.) # adding an outermost radius in order to cut off the dust filled part far outside the moon gaps. This determines to what extent the Hill sphere is filled. If the argument is (transit_days/2.), then the full Hill sphere is filled. If it is (transit_days/4.), then only half of the Hill sphere (until 1/2 R_H) is filled.
    ...
    taun_rings = np.resize([5.2,100.],len(radii)) # needs to have as many entries as radii list has, np.resize does that
    ...
    eclipse_center = days_array[int(ecl_cen)] # it should be 0 here, but it's not exactly zero. Not sure if the result is close enough though.
    ...
    res[3] = 90       # phi_deg [degrees]: angle between the normal of the secondary companion's orbit and the normal of the ring plane
    ...
    print 'Orbital velocity = %5.2f  km/s (use option -s to set new velocity) - option not available in this version' % (v/1.e3)
    ...
    # export
    conv_light = Table([g[0], g[1], g_err], names=('time', 'flux', 'flux_rms')) #flux_rms still there cause I'll need it at the readout in bring_lightcurves.py
    ...
    theta = np.linspace(0.,2*theta_lim,(2*theta_lim/step_size)+1)   # +1 because I need an uneven number of entries
    theta = theta - (np.max(theta)/2.)      # centering the theta values on zero
    
    sc_time,sc_light = bring.forward_scattering2(res[2],res[3],rad_rings, exorings.y_to_tau(taun_rings),res[0],theta,g_fac,v_orb,kern,dstar)
